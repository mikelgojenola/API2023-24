...
public class Juego extends Observable{
	...
	public static void jugarRonda2(ArrayList<Integer> jokaldiCartak) {
		...
		if(getLista()[m].getPuntos() == 3) {
			...
			getLista()[j].hacerJugada2(jokaldiCartak, getLista()[m].getPlatoKop() >= 1); // Le pasamos sus cartas seleccionadas y si el rival tiene alg�n plato.
			if(!getLista()[j].noCombinaciones()){
				...
				if (getLista()[j].getPuntos() != 3) {
					getLista()[m].hacerJugada(getLista()[j].getPlatoKop() >= 1); // Le pasamos si el rival tiene alg�n plato.
